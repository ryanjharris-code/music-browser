...
let allMusic = [], currentGenre = null, currentSubgenre = "";
const content = document.getElementById("content");
const subgenreButtons = document.getElementById("subgenreButtons");
...
function showHomePage() {
  ...
  genres.forEach(genre => {
    ...
    btn.onclick = () => { window.location.hash = `#genre=${encodeURIComponent(genre)}`; };
...
function showGenrePage(genre) {
  ...
  subgenreButtons.appendChild(allBtn);
  ...
  subgenres.forEach(sg => {
    ...
    btn.onclick = () => {
      currentSubgenre = sg;
      renderMusicCards(genre, sg);
    };
    subgenreButtons.appendChild(btn);
  });
